...
public/
  ...
  favicon.svg
  og.svg                        # legacy source SVG (kept for reference; build-og.mjs templates inline)
  og.png                        # OG for /
  og-about.png                  # OG for /about
...
- The top nav highlights the current top-level page with a gradient underline (`aria-current="page"` + CSS).
- Below 720px the email CTA in the nav swaps the long address for a compact `EMAIL →` label.

## Accessibility & motion

- Skip-to-content link.
- All animations honor `prefers-reduced-motion: reduce` (including the console-mock animations).
- Interactive elements are keyboard-reachable with visible focus rings (FAQ summaries get an explicit cyan outline).
- Every "Book a consultation" / mailto CTA carries an `aria-label` containing the email address, plus a visually-hidden `<span class="sr-only">contact@svlab.online</span>` so the address is recoverable even if the JS-based Cloudflare email obfuscation script fails to run.
